...
public class PlayerTempTickC2SPacket {
	...
	public static void receive(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler,
		...
		Biome.Precipitation precip = player.getWorld().getBiome(player.getBlockPos()).value()
				.getPrecipitation(player.getBlockPos());
		...
		heatBlockBox.forEach((state) -> {
			ThermMod.config.heatingBlocks.forEach((b, t) -> {
				if (Objects.equals(state.toString(), b)) {
					if (state.isOf(Blocks.CAMPFIRE) || state.isOf(Blocks.SOUL_CAMPFIRE)) { // hard code to keep unlit
																							// campfires from heating.
				...
				} else if (Objects.equals(state.getBlock().toString(), b)
						&& !Objects.equals(state.toString(), state.getBlock().toString())) {
					playerState.restingTemp += t;
				}
			});
		...
		if (playerState.searchFireplaceTick <= 0) {
			...
			if (ThermMod.config.enableWind) {
				if (ThermMod.config.multidimensionalWind || dim.natural()) {
					...
					playerState.windTemp = playerState.baseWindTemp
							* ((double) unblockedRays / ThermMod.config.windRayCount);
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < player.getInventory().getArmorStack(i).getEnchantments().size(); j++) {
				if (Objects.equals(
						player.getInventory().getArmorStack(i).getEnchantments().getCompound(j).getString("id"),
		...
		if (playerState.temp <= ThermMod.config.freezeThreshold1
				&& playerState.temp > ThermMod.config.freezeThreshold2) {
		...
		} else if (playerState.temp >= ThermMod.config.burnThreshold1
				&& playerState.temp < ThermMod.config.burnThreshold2) {
		...
		if (Objects.equals(playerState.damageType, "freeze")) {
			...
			boolean res = false;
			try {
				String fireRes = Objects
						.requireNonNull(player.getStatusEffect(
								new StatusEffectInstance(StatusEffects.FIRE_RESISTANCE).getEffectType()))
						.getEffectType().getName().getString();
				res = true;
		...
		// ThermMod.LOGGER.info("p: " +
		// player.getWorld().getBiome(player.getBlockPos()).value().getPrecipitation(player.getBlockPos())
		// + " rain?: " + player.getWorld().isRaining());
	}
