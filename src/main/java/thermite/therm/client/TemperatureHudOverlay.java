...
public class TemperatureHudOverlay implements HudRenderCallback {
	...
	public static final Identifier TEMPERATURE_EXTREME_OVERLAY = new Identifier(ThermMod.modid,
			"textures/misc/temp_extreme_1.png");
	public static final Identifier TEMPERATURE_EXTREME_OVERLAY2 = new Identifier(ThermMod.modid,
			"textures/misc/temp_extreme_2.png");
	...
	public void onHudRender(DrawContext drawContext, float tickDelta) {
		if (ThermClient.showGui) {
			if (Objects.equals(ThermMod.config.temperatureDisplayType, "gauge")) {
				...
				drawContext.drawTexture(THERMOMETER_GAUGE, x - ((44 + 149) - Math.round(2 * pixelMultiplier)),
				...
				if (ThermClient.clientStoredTempDir > 0) {
					drawContext.drawTexture(THERMOMETER_FLAME, x - (17 + 149), y - (Math.round(22 * pixelMultiplier)),
					...
					drawContext.drawTexture(THERMOMETER_SNOWFLAKE, x - (17 + 149),
							y - (Math.round(22 * pixelMultiplier)), 0, 0, Math.round(8 * pixelMultiplier),
							Math.round(8 * pixelMultiplier), Math.round(8 * pixelMultiplier),
							Math.round(8 * pixelMultiplier));
				} else {
					drawContext.drawTexture(THERMOMETER_STILL, x - (17 + 149), y - (Math.round(22 * pixelMultiplier)),
							0, 0, Math.round(8 * pixelMultiplier), Math.round(8 * pixelMultiplier),
							Math.round(8 * pixelMultiplier), Math.round(8 * pixelMultiplier));
				}

				if (offHand.getItem() == ThermMod.THERMOMETER_ITEM) {
					drawContext.drawTexture(THERMOMETER_DISPLAY, (x - (x - 16)) + ThermMod.config.thermometerXPos,
							frameY + ThermMod.config.thermometerYPos, 0, 0, Math.round(16 * pixelMultiplier),
							Math.round(13 * pixelMultiplier), Math.round(16 * pixelMultiplier),
							Math.round(13 * pixelMultiplier));
					assert client != null;
					drawContext.drawText(client.textRenderer, "§7" + ThermClient.clientStoredTemperature,
							((x - (x - 16)) + 6) + ThermMod.config.thermometerXPos,
				...
				if (!client.player.isSpectator() && !client.player.isCreative()) {
					...
					} else if (temp < ThermMod.config.freezeThreshold1 + 11
							&& temp > ThermMod.config.freezeThreshold1 + 5) {
					...
					} else if (temp > ThermMod.config.burnThreshold1 - 11
							&& temp < ThermMod.config.burnThreshold1 - 5) {
				...
				if (offHand.isOf(ThermMod.THERMOMETER_ITEM)) {
					drawContext.drawTexture(THERMOMETER_DISPLAY, (tx - (tx - 16)) + ThermMod.config.thermometerXPos,
							tFrameY + ThermMod.config.thermometerYPos, 0, 0, Math.round(16 * pixelMultiplier),
							Math.round(13 * pixelMultiplier), Math.round(16 * pixelMultiplier),
							Math.round(13 * pixelMultiplier));
					assert client != null;
					drawContext.drawText(client.textRenderer, "§7" + ThermClient.clientStoredTemperature,
	...
	private void renderOverlay(DrawContext context, Identifier texture, float opacity, int width, int height, float r,
			float g, float b) {
