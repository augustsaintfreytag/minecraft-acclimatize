...
public class FireplaceBlock extends BlockWithEntity {
	...
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state,
		...
		return FireplaceBlock.checkType(type, ThermMod.FIREPLACE_BLOCK_ENTITY,
				(world1, pos, state1, be) -> FireplaceBlockEntity.tick(world1, pos, state1, be));
	}
	...
	public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
		...
		if (state.get(LIT)) {
			for (int i = 0; i < 10; i++) {
				if (!Objects.equals(world.getBlockState(new BlockPos(pos.getX(), pos.getY() + 1 + i, pos.getZ()))
						.getBlock().getName(), Blocks.BRICK_WALL.getName())
						&& !Objects.equals(world.getBlockState(new BlockPos(pos.getX(), pos.getY() + 1 + i, pos.getZ()))
								.getBlock().getName(), Blocks.COBBLESTONE_WALL.getName())
						&& !Objects.equals(world.getBlockState(new BlockPos(pos.getX(), pos.getY() + 1 + i, pos.getZ()))
								.getBlock().getName(), Blocks.STONE_BRICK_WALL.getName())) {
					if (i == 0) {
						world.addParticle(ParticleTypes.CAMPFIRE_COSY_SMOKE, (double) pos.getX() + 0.5,
								(double) pos.getY() + 1, (double) pos.getZ() + 0.5, 0.0, 0.03125, 0.0);
						break;
					} else if (i != 0) {
						world.addParticle(ParticleTypes.CAMPFIRE_COSY_SMOKE, (double) pos.getX() + 0.5,
								(double) pos.getY() + 1 + i, (double) pos.getZ() + 0.5, 0.0, 0.03125, 0.0);
						break;
					}
				}
			}
